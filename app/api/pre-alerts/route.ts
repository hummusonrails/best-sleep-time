  ...
  try {
    ...
    try {
      const result = await db.execute({
        sql: "SELECT id, timestamp, title_he, body_he, city_ids, regions, alert_type, created_at FROM pre_alerts WHERE timestamp > ? ORDER BY timestamp DESC LIMIT 500",
        args: [since],
      });
      rows = result.rows;
    } catch {
    ...
    const preAlerts: PreAlert[] = rows.map((r) => ({
      id: r.id as string,
      ...
      alert_type: r.alert_type as "early_warning" | "exit_notification",
